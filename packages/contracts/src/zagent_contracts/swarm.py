"""Swarm contracts (plan §4 — width swarm).
...
and per-lane prompt authoring are the Lead's job, never the user's. When the
...
class SwarmSlice(BaseModel):
    ...
    title: str
    prompt: str = Field(description="Lead-authored per-lane task slice")
...
class Decomposition(BaseModel):
    """Structured-output target for the swarm blueprint's decompose node."""

    schema_version: int = SCHEMA_VERSION
    slices: list[SwarmSlice] = Field(default_factory=list)
    counter_proposal: int | None = Field(
        default=None,
        description="Lead's proposed lane count when the requested count is wasteful",
    )
    rationale: str = ""
