#!/usr/bin/env bash
# Collegium VM deploy — the whole stack in two steps (Ubuntu 24.04):
#
...
DATA_ROOT="$(grep -E '^COLLEGIUM_DATA_ROOT=' .env | cut -d= -f2- | tr -d '"' || true)"
DATA_ROOT="${DATA_ROOT:-/srv/collegium}"
...
docker build -f ../../worker/Dockerfile -t collegium-worker:0.1.0 ../..
...
echo
echo "==> collegium is up:"
echo "    local:   http://localhost:8080"
echo "    login:   \$COLLEGIUM_BOOTSTRAP_ADMIN_USERNAME / \$COLLEGIUM_BOOTSTRAP_ADMIN_PIN from .env"
echo
