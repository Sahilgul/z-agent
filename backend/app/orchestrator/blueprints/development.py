...
develop   (agentic developer lane): one developer lane with the approved plan steps
            ...
            lane reports (deterministic fallback marks them done on lane completion).
...
evaluate  (agentic evaluator lane, FRESH context): a SEPARATE lane/session — never
            the develop lane's session — that re-reads the workspace from scratch
...
class DevelopmentBlueprint(Blueprint):
    ...
    async def _develop(self, ctx: BlueprintContext) -> None:
        ...
        # the developer lane completes. (The lane's own step reports override this
    ...
    async def _evaluate(self, ctx: BlueprintContext) -> None:
        """Fresh-context evaluator: a brand-new lane/session that re-reads the
        workspace from scratch and verifies each step's success_criterion AGAINST
        the stored stamp evidence. It must NOT share the developer lane's session —
        ...
        ctx.artifacts["evaluator_lane_id"] = lane.id
        await self._await_lane(lane.id)
        ctx.artifacts["evaluator_notes"] = self._last_message_text(lane.id) or ""
    ...
    async def _stamp(self, ctx: BlueprintContext) -> None:
        ...
        repo: Repo = ctx.artifacts["repo_row"]
        develop_lane_id = ctx.artifacts.get("develop_lane_id") or ctx.run.id
        ...
        try:
            run = session.get(Run, ctx.run.id)
            lane = session.get(Lane, develop_lane_id)
            seq = (lane.next_seq if lane else 0)
            session.add(Event(
                run_id=run.id, lane_id=develop_lane_id, seq=seq,
            ...
            if screenshots:
                session.add(Event(
                    run_id=run.id, lane_id=develop_lane_id, seq=seq + 1,
                    type="screenshot", title="playwright screenshots",
                    payload={"routes": screenshots},
                ))
            if lane:
                lane.next_seq = seq + (2 if screenshots else 1)
            run.last_active_at = datetime.now(timezone.utc)
            session.commit()
            ctx.run = run
        finally:
    ...
    def _persist_trajectory(self, ctx: BlueprintContext) -> None:
        ...
        try:
            ...
            session.add(TrajectorySummary(
                run_id=run.id, lane_id=develop_lane_id, user_id=run.created_by,
