...
def _volume(tmp_path):
    vol = tmp_path / "sessions" / "run-1" / "lane-1"
...
def test_upload_materialize_purge_cycle(tmp_path):
    vol = _volume(tmp_path)
    client = DictClient()
    assert session_store.upload("run-1", "lane-1", vol, client=client) is True
    assert "run-1/lane-1.tar.gz" in client.blobs
    dest = tmp_path / "other-host"
    assert session_store.materialize("run-1", "lane-1", dest, client=client) is True
    assert (dest / "session.jsonl").exists()
    assert session_store.purge("run-1", "lane-1", client=client) is True
    assert client.blobs == {}
...
def test_materialize_miss_returns_false(tmp_path):
    assert session_store.materialize("ghost", "lane", tmp_path / "d",
