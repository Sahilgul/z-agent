...
def test_seed_creates_demo_run_with_events_and_lane(session):
    seed_users.seed()
    ...
    lane = session.query(Lane).filter_by(run_id=run.id).one()
    assert lane.status == "completed"
    assert lane.next_seq == len(seed_users.DEMO_EVENTS)
