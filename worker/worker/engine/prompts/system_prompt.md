...
- **Symbol lookup chain**: to find where a function/class/symbol is defined or used, follow the golden chain — (1) `code_search` for the exact symbol name (e.g. `def name` / `class name` / the call site) to get `file:line` hits; (2) `file_read` at that offset with the SMALLEST range that covers the symbol (use offset/limit — never whole-file reads for a single symbol). Never start from a broad grep over the whole repo when you already know the symbol name, and never read a 2000-line file to answer a question about one function.
