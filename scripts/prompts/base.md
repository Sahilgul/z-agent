...
- Everything between `--- PROMPT START ---` and `--- PROMPT END ---` is the byte-stable base. It MUST NOT contain per-turn values.
- `{{SKILLS_LISTING}}` is the only slot in the base; the registry replaces it with the versioned skill pointer list (platform < team/repo < per-user overlay), or removes that block entirely when no skills are synced.
- The registry then appends exactly ONE model suffix (Appendix A or B) chosen by LiteLLM model id, producing the single system message. ONE system message per turn, always — open-weight models 400 on two consecutive system messages (vexa production lesson).
- Per-turn content (env block, AGENTS.md diff stream, memory slices, mode envelopes) rides as separate user-role messages — see Appendix C. Never inline them here.
- Soft budget: base ≤ ~3K tokens. Every line must be traceable to an observed failure mode (the vexa bar).
...
## Appendix A — Model suffix `default-open.md` (weaker open-weight models)

Appended after the base + skills listing when the resolved LiteLLM model is a general open-weight model. Reinforcement only — deliberately contains nothing the base doesn't teach, phrased for models that need policy restated as tagged rules.
...
them, use lookup tools for identifiers, and treat a successful tool result —
never your intention — as the only valid basis for saying something is done.
</grounding>

<loop_control>
If a tool call fails, read the error and change the call before retrying; never
re-issue the same failing call unchanged. If the same error repeats, reduce the
input, add a termination condition, or switch approaches before trying again.
</loop_control>

<final_answer>
After tool calls succeed, the final answer must include the concrete result, not
just "done". Preserve the exact identifiers, counts, paths, and versions from the
tool output. If you used an opaque ID and a name/details lookup exists, resolve
it to human-readable form before answering.
</final_answer>

<followups>
Ask follow-up questions only for information needed to proceed safely or
correctly. Do not re-ask for constraints the human already gave.
</followups>
```

## Appendix B — Model suffix `kimi.md` (Kimi-family models)

Kimi models are already strong at tool calling; keep the suffix minimal.

```xml
<grounding>
Verify state with tools instead of recalling it. Treat a successful tool result —
never your intention — as the only valid basis for saying something is done.
</grounding>

<loop_control>
If a tool call fails, read the error and change the call before retrying; never
re-issue the same failing call unchanged.
</loop_control>
```

## Appendix C — Per-turn content (NOT part of this file; rendered by the registry as user-role messages)

- **Env block** (every turn): `<environment_context>` XML carrying thread id, repo, branch, base SHA, drift status, mode, autonomy, budget, date — the values Section "Your environment" refers to.
- **AGENTS.md diff stream**: full content on first turn (`<INSTRUCTIONS>` wrapper); on change, a replacement notice ("These AGENTS.md instructions replace all previously provided ones"); on removal, a removal notice.
- **Memory slices**: thread artifact (global slice every turn) + scoped team knowledge for this repo/user.
- **Mode envelope**: on mode change, a `<system-envelope><mode>…` message carrying the mode's protocol + exit condition, paired with a tool-filter change on the API call. Envelope templates live in `worker/engine/prompts/envelopes/{ask,plan,development,debug,goal}.md` (next artifacts, per Rounds 19/21 — `goal.md` additionally carries the long-horizon continuation contract).
- **One-shot nudges**: drift, collision, approval outcomes, compaction notices — same `<system-envelope>` wrapper, emitted by the platform services that own them.
